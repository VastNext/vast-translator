...
- 🧱 Provider 卡片可独立首次翻译、重新翻译或失败重试；临时增加服务不会重复调用已有结果
...
勾选 Provider 后会立即加入一张“尚未翻译”卡片，但不会自动产生请求。可在卡片上点击“翻译”，成功后可“重新翻译”，失败后可“重试”；这些操作只调用该卡片对应的 Provider，并使用点击时输入框中的文本和语言。底部“全部重新翻译”用于显式更新所有已选 Provider。取消勾选会移除对应卡片并取消其未完成请求。

Provider 选择会保存在当前浏览器，并在其他标签页修改后同步。恢复选择只会创建“尚未翻译”卡片，不会自动发起翻译；未知或已下架的 Provider 会被忽略。
